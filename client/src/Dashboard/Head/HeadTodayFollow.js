...
const HeadTodayFollow = () => {
    ...
    const handleSearch = () => {
        const fData = htotalTodayFUp?.filter(si => si.course.name === courseRef.current.value || si.batch.name === batchRef.current.value || si.head.name === headRef.current.value || si.user.name === userRef.current.value)
        setFilterData(fData)
    };
    ...
    return (
        <div className='mx-2 my-6'>
            <h3 className="text-2xl mb-3">{user.name}'s Today Followup Students : {htotalTodayFUp.length}</h3>
