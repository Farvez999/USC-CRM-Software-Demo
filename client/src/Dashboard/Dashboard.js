...
const Dashboard = () => {
    ...
    //         {
    //             label: 'Dataset 1',
    //             data: '5-5-23,9-5-23,11-5-23,200,200,200,200',
    ...
    const { data: allleads = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            if (user._id) {
                if (user.role === 'admin') {
                    const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads`);
                    const data = await res.json();
                    setFilterData(data)
                    return data;
                }
                else if (user.role === 'head') {
                    const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads/?head.name=${user.name}`);
                    const data = await res.json();
                    setFilterData(data)
                    return data;
                }
                else {
                    const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?user.name=${user.name}`);
                    const data = await res.json();
                    setFilterData(data)
                    return data;
                }
    ...
    const { data: admissions = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?admission=true`);
                const data = await res.json();
                setAddmissionData(data)
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?admission=true&head.name=${user.name}`);
                const data = await res.json();
                setAddmissionData(data)
                return data;
            }
    ...
    const { data: closes = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?close=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?close=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
    ...
    const { data: onlines = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?onlineInterested=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?onlineInterested=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
    ...
    const { data: offlines = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?offlineInterested=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?offlineInterested=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
    ...
    const { data: seminarInteresteds = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?seminarInterested=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?seminarInterested=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?seminarInterested=true&user.name=${user.name}`, {
    ...
    const { data: seminarAttends = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?seminarAttend=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?seminarAttend=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
    ...
    const { data: noReceives = [] } = useQuery({
        ...
        queryFn: async () => {
            if (user.role === 'admin') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?noReceive=true`);
                const data = await res.json();
                return data;
            }
            else if (user.role === 'head') {
                const res = await fetch(`https://demo-usc-crm-software.vercel.app/leads?noReceive=true&head.name=${user.name}`);
                const data = await res.json();
                return data;
            }
            else {
    ...
    return (
        <div className='mx-2 my-6'>
            ...
            >
                <h2 className='text-2xl font-bold'>Hello {user.name} ! {greeting} ! {today} !</h2>
            </div>
            ...
            <div className='grid mt-6 gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-6'>
                ...
                <div className={`card text-white p-6 md:card-side shadow-xl bg-accent`}>
                    ...
                    <div className="card-body">
                        ...
                        <h1 className="card-title justify-center">{addmissionData.length}</h1>
                    </div>
                </div>

                <div className={`card text-white p-6 md:card-side shadow-xl bg-gradient-to-r from-secondary to-primary`}>
                    <figure>
                ...
                <div className={`card text-white p-6 md:card-side shadow-xl bg-gradient-to-r from-secondary to-primary`}>
                    ...
                    <div className="card-body">
                        ...
                        <h1 className="card-title justify-center">{todayFollowup.length}</h1>
                    </div>
                </div>
                ...
                <div className={`card text-white p-6 md:card-side shadow-xl bg-gradient-to-r from-primary to-secondary`}>
                    ...
                    <div className="card-body ">
                        ...
                        <h1 className="card-title justify-center">{seminarInteresteds.length}</h1>
                    </div>
                </div>
                ...
                <div className={`card text-white p-6 md:card-side shadow-xl bg-accent`}>
                    ...
                    <div className="card-body">
                        ...
                        <h1 className="card-title justify-center">{seminarAttends.length}</h1>
                    </div>
                </div>
