...
const TotalSemiAttend = () => {
    ...
    const { data: totalSemiAttend = [], refetch } = useQuery({
        queryKey: ['totalSemiAttend',],
        queryFn: async () => {
    ...
    const handleSearch = () => {
        const fData = totalSemiAttend?.filter(si => si.course.name === courseRef.current.value || si.batch.name === batchRef.current.value || si.head.name === headRef.current.value || si.user.name === userRef.current.value)
        setFilterData(fData)
    };
    ...
    const handleInputChange = event => {
        ...
        console.log(value);
        const fiData = totalSemiAttend.filter(si => formatedDate(si.createdAt) === value)
            ...
            <nav aria-label="breadcrumb" className="w-full p-4 dark:bg-gray-800 dark:text-gray-100">
                <ol className="flex h-8 space-x-2">
                    ...
                    <li className="flex items-center space-x-2">
                        ...
                        <a rel="noopener noreferrer" href="#" className="flex items-center px-1 capitalize hover:underline">Total Seminar Attends Students</a>
                    </li>
                </ol>
            </nav>
                    ...
                    <form>
                        <table className="table w-full">
                            ...
                            <tbody className='w-fit text-xs'>
                                {
                                    filterData.length > 0 &&
                                    filterData?.filter(semiattend => {
                                        return search?.toLowerCase() === '' ? semiattend : semiattend.name.toLowerCase().includes(search?.toLowerCase()) || semiattend?.email?.toLowerCase().includes(search?.toLowerCase()) || semiattend.phone.toLowerCase().includes(search?.toLowerCase())
                                    })?.map((semiattend, i) =>
                                        <tr key={semiattend._id}>
                                            <th className='p-1 border-2'>{i + 1}</th>
                                            <td className='p-1 border-2'>{semiattend.createdAt.slice(0, -14)}</td>
                                            <td className='p-1 border-2'>{semiattend.course.name}</td>
                                            <td className='p-1 border-2'>{semiattend.batch.name}</td>
                                            ...
                                            <td className='p-1 border-2'>
                                                <p className='btn btn-xs btn-denger' onClick={() => handleDelete(semiattend._id)} >Delete</p>
                                            </td>
                                        </tr>
                                    )
                                }
