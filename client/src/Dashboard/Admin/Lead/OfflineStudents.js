...
const OfflineStudents = () => {
    ...
    const handleSearch = () => {
        const fData = totalOffline?.filter(si => si.course.name === courseRef.current.value || si.batch.name === batchRef.current.value || si.head.name === headRef.current.value || si.user.name === userRef.current.value)
        setFilterData(fData)
    };
            ...
            <nav aria-label="breadcrumb" className="w-full p-4 dark:bg-gray-800 dark:text-gray-100">
                <ol className="flex h-8 space-x-2">
                    ...
                    <li className="flex items-center space-x-2">
                        ...
                        <a rel="noopener noreferrer" href="#" className="flex items-center px-1 capitalize hover:underline">Total Offline Students</a>
                    </li>
                </ol>
            </nav>
                            ...
                            <tbody className='w-fit text-xs'>
                                {
                                    ...
                                    filterData?.filter(offline => {
                                        return search?.toLowerCase() === '' ? offline : offline.name.toLowerCase().includes(search?.toLowerCase()) || offline.email?.toLowerCase().includes(search?.toLowerCase()) || offline.phone.toLowerCase().includes(search?.toLowerCase())
                                    })?.map((offline, i) =>
                                        <tr key={offline._id}>
                                            ...
                                            <td className='p-1 border-2'>
                                                <p className='btn btn-xs btn-denger' onClick={() => handleDelete(offline._id)} >Delete</p>
                                            </td>
                                        </tr>
                                    )
                                }
