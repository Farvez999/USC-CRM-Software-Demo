...
const OnlineStudents = () => {
    ...
    const { data: totalOnline = [], refetch } = useQuery({
        queryKey: ['totalOnline',],
        queryFn: async () => {
    ...
    const handleDelete = (online) => {
        ...
        fetch(`https://demo-usc-crm-server.vercel.app/delete/${online}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleSearch = () => {
        const fData = totalOnline?.filter(si => si.course.name === courseRef.current.value || si.batch.name === batchRef.current.value || si.head.name === headRef.current.value || si.user.name === userRef.current.value)
        setFilterData(fData)
    };
    ...
    const handleInputChange = event => {
        ...
        console.log(value);
        const fiData = totalOnline.filter(si => formatedDate(si.createdAt) === value)
            ...
            <nav aria-label="breadcrumb" className="w-full p-4 dark:bg-gray-800 dark:text-gray-100">
                <ol className="flex h-8 space-x-2">
                    ...
                    <li className="flex items-center space-x-2">
                        ...
                        <a rel="noopener noreferrer" href="#" className="flex items-center px-1 capitalize hover:underline">Total Online Students</a>
                    </li>
                </ol>
            </nav>
                            ...
                            <tbody className='w-fit text-xs'>
                                {
                                    ...
                                    filterData?.filter(online => {
                                        return search?.toLowerCase() === '' ? online : online.name.toLowerCase().includes(search?.toLowerCase()) || online.email?.toLowerCase().includes(search?.toLowerCase()) || online.phone.toLowerCase().includes(search?.toLowerCase())
                                    })?.map((online, i) =>
                                        <tr key={online._id}>
