...
const TotalNoReceive = () => {
    ...
    const { data: totalNoReceive = [], refetch } = useQuery({
        queryKey: ['totalNoReceive',],
        queryFn: async () => {
    ...
    const handleDelete = (noreceive) => {
        ...
        fetch(`https://demo-usc-crm-server.vercel.app/delete/${noreceive}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleSearch = () => {
        const fData = totalNoReceive?.filter(si => si.course.name === courseRef.current.value || si.batch.name === batchRef.current.value || si.head.name === headRef.current.value || si.user.name === userRef.current.value)
        setFilterData(fData)
    };
    ...
    const handleInputChange = event => {
        ...
        console.log(value);
        const fiData = totalNoReceive.filter(si => formatedDate(si.createdAt) === value)
            ...
            <nav aria-label="breadcrumb" className="w-full p-4 dark:bg-gray-800 dark:text-gray-100">
                <ol className="flex h-8 space-x-2">
                    ...
                    <li className="flex items-center space-x-2">
                        ...
                        <a rel="noopener noreferrer" href="#" className="flex items-center px-1 capitalize hover:underline">Total No Receive Students</a>
                    </li>
                </ol>
            </nav>
                            ...
                            <tbody className='w-fit text-xs'>
                                {
                                    ...
                                    filterData?.filter(noreceive => {
                                        return search?.toLowerCase() === '' ? noreceive : noreceive.name.toLowerCase().includes(search?.toLowerCase()) || noreceive?.email?.toLowerCase().includes(search?.toLowerCase()) || noreceive.phone.toLowerCase().includes(search?.toLowerCase())
                                    })?.map((noreceive, i) =>
                                        <tr key={noreceive._id}>
                                            ...
                                            <td className='p-1 border-2'>
                                                <p className='btn btn-xs btn-denger' onClick={() => handleDelete(noreceive._id)} >Delete</p>
                                            </td>
                                        </tr>
                                    )
                                }
